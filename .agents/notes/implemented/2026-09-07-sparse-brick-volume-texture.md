...
Superseded on 2026-09-26: the 2.75 power matched the dense grid, but the dense grid was already far more transparent than the instanced renderer because the floorless cubic ramp and the intensity-scaled opacity ceiling both suppressed light echo. The ramp is now `0.1 + 0.9·t²`, the light-echo ceiling is linear in the slider (~0.33 at default), and extinction fades in over the first 5 NM from the camera; see `docs/rendering-weather-volume.md`.
