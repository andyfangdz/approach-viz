...
function main() {
  ...
  // Validate required reference data before replacing an existing database.
  const minimumsDb = parseApproachReferenceSource(fs.readFileSync(APPROACH_DB_PATH, 'utf8'));

  fs.mkdirSync(DB_DIR, { recursive: true });
  const buildPath = `${DB_PATH}.building-${process.pid}`;
  ...
  `);
    ...
    db.close();
    fs.renameSync(buildPath, DB_PATH);
