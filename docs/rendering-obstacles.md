...
- **Chart-significant obstacles bypass the threshold.** Following the FAA Chart User's Guide TPP plan-view rule ("any obstacle which penetrates a slope of 67:1 emanating from any point along the centerline of any runway shall be considered for charting"), the loader always includes obstacles whose height above the airport elevation exceeds `distance / 67` measured to the nearest runway centerline point (`lib/obstacles/plate-significance.ts`: reciprocal threshold pairs form centerline segments, unpaired ends count as points, and airports with no runway rows fall back to the airport reference point). This keeps controlling obstacles visible — e.g. the KSBS `8353±` tower, only 102 ft AGL but 1,471 ft above the field on a ridge 3.3 NM out.
- **Below-threshold penetrators are decluttered TPP-style.** The 67:1 rule only makes obstacles "considered for charting"; around airports with nearby high terrain it would otherwise sweep in every rooftop on a ridge (KCDW: 232 sub-200 ft AGL penetrators on the Watchung ridges, where the plate charts a handful). `declutterLocalHighest` keeps a below-threshold penetrator only when nothing at least as tall (AMSL) is already shown within 1 NM (`OBSTACLE_CHART_DECLUTTER_RADIUS_NM`), collapsing clusters to their locally-highest obstacle — KCDW's extras drop to 18, and KSBS keeps `8353±` while its two shorter same-ridge neighbors drop, matching the plate. Obstacles passing the user threshold are never decluttered. When the response cap applies, the kept penetrators survive preferentially, then the tallest by AMSL.
