...
- The drape mesh is built in the scene-geometry worker (`app/scene/nexrad/nexrad-drape.ts`), once per grid rather than per poll; until it arrives, and whenever the drape mode is flat, the small flat/curved mesh is built where it is drawn. The raster is used only when the drape mode is `terrain`.
...
- The WASM `decode_and_prepare_mrms` call joins the prepare-pass outputs with payload columns inside Rust and rasterizes the selected bricks into a sparse RG8 volume (`crates/approach-viz-core/src/mrms_render.rs::build_volume_texture`): a page table over the logical grid (one RG8 entry per `8^3`-texel page holding `slot + 1` little-endian, or `0` when the page holds no echo) plus a pool of resident bricks, each the `8^3` core wrapped in a one-texel apron copied from the neighboring logical texels (clamped at the grid edge) so trilinear filtering never leaves the brick. The logical grid is never materialized. Per logical texel `R` is whole dBZ (`0` = empty, overlaps resolve strongest-return-wins) and `G` is the phase code (bled into adjacent empty texels — every stored copy, core and apron — so trilinear filtering does not tint echo edges toward rain); altitudes are corrected feet (earth-curvature drop pre-applied), and the result carries placement metadata (`origin`/`cellSize`/`baseFeet`/`binSizeFeet`, `coarsenX`/`coarsenZ`), page/pool dimensions and `brickCount`, plus altitude-guide extents (`maxAbsXNm`/`maxAbsZNm`/`maxCorrectedTopFeet`). The horizontal grid keeps full source resolution (a 120 NM request is about 440 cells per axis, so `coarsenX/Z = 1`) up to a 2048-cell logical axis (256 pages); resident bricks are capped at `MAX_VOLUME_BRICKS = 8192` (a 16.4 MB pool), and past that the grid coarsens by the next whole footprint multiple and the selection is re-counted before anything is allocated, so the pool is bounded without failing. The pool is laid out 25 bricks per row/column (250 texels, within WebGL2's guaranteed 256-texel 3D texture axis) and grows one brick layer at a time. Altitude bins target 500 ft (finer than the lowest MRMS level spacing) capped at 96 bins. The native iOS/macOS app still consumes `build_render_volume` flat columns through UniFFI for its instanced renderer.
- The poll response moves the page table and brick pool to the main thread as Comlink transferables (zero-copy), alongside volume metadata for the debug panel, the optional cross-section grid, and prepared echo-top surfaces + summary metadata for caps/debug readouts.
...
- The reflectivity volume renders as a single box mesh whose fragment shader raymarches the sparse page-table + brick-pool volume front-to-back (`app/scene/nexrad/NexradVolumeRaymarch.tsx`): draw cost no longer scales with voxel count, replacing the former base+glow instanced-mesh pair (~2 GPU instances per brick). The page table and pool upload as two RG8 3D textures (nearest and trilinear respectively). At each step the ray fetches the page for its position (`texelFetch`): an **empty page is jumped in one iteration** to the page's exit face (a one-level DDA, the NanoVDB HDDA idea with one internal level), landing back on the jittered sample lattice so the cadence does not change at page entries; a resident page is sampled at pool position `brick_origin + 1 + (texel - page * 8)`, which the apron keeps inside the brick. Empty air therefore costs one iteration per 8-texel page instead of one sample per texel, and the loop's 384-iteration ceiling is a bound, not a budget the ray spends on air.
...
- The mesh renders back faces with the hardware depth test off so the camera can sit inside the weather volume (rays clamp to the camera when it is inside the box). A depth test at the ray's exit point would let anything between the camera and the far wall — a terrain wireframe line, a ridge under the box floor — discard the whole ray. Terrain occlusion is done per sample instead: in the opaque-ground surface modes (satellite / 3D map) the overlay loads the same z8 Terrarium raster the mosaic drapes on — one `useElevationRaster` call owned by `NexradVolumeOverlay`, keyed to the reference point, which loads the raster into the scene-geometry worker once for both the raymarch and the mosaic so enabling both never fetches or decodes the tiles twice. In that worker, `buildGroundHeightfield` samples the ground under every volume column into the texture's normalized altitude frame (curvature drop subtracted when compensation is on, so terrain and weather share one corrected frame), and the shader ends a ray where it enters the ground — nothing behind a ridge is visible. Page skipping keeps that guarantee: `buildGroundPageMax` (`app/scene/nexrad/nexrad-ground.ts`) records the highest ground under each page column (with a one-column halo, because the heightfield texture is linearly filtered), uploaded as a nearest-filtered R32F texture. Both depend only on the grid geometry, so they are computed once per grid rather than per poll, and the raymarch takes them through a `VolumeGroundSource` (the volume smoke test supplies synthetic terrain the same way). An empty page is jumped only when its bottom sits at or above that value; an empty page that may hold a ridge is stepped through with the per-sample ground test and no volume fetch. The debug panel reports `Volume Ground` (`none` / `terrain-loading` / `terrain` / `terrain-unavailable`); a failed raster is reported, and the volume marches unclipped rather than passing a flat guess off as terrain. In the translucent-surface modes (terrain wireframe, chart map, plate) no occlusion applies and the weather overlays the surface. Other opaque geometry inside the volume (approach path prisms, aircraft) is overlaid by the translucent weather rather than occluding it.
- Per-sample color comes from a nearest-filtered `(band x phase)` LUT texture built from the shared band tables (`buildDbzPhaseLutTexture`), so the discrete legend bands stay crisp inside the volume. Per-sample extinction is a 0.1 floor plus a quadratic over the 5–65 dBZ span (`0.1 + 0.9·t²`, gated to zero below ~5 dBZ so trilinear falloff into empty texels fades cleanly), so light and moderate precipitation has a visible body while a 60 dBZ core is still about five times denser than 20 dBZ echo. At the default 35% opacity a 3 NM deep 30 dBZ column reads ~39% opaque and a 3 NM 50 dBZ core ~70%. Keeping a thick light shell from burying the core behind it is left to the opacity ceiling below rather than to the ramp: the ramp was previously a floorless cubic (later a 2.75 power, recalibrated for source resolution) that did both jobs at once, which left 20 dBZ echo around 1/16 and 30 dBZ echo around 1/5 of the per-cell opacity the instanced renderer had, so light and moderate weather read as nearly invisible.
- Each ray also carries an **opacity ceiling that rises with echo intensity** (`opacityCap` in the shader): the accumulated alpha may not exceed the ceiling for the intensity being sampled, which ramps from a light-echo cap (`uLightOpacityCap`, 0.08–0.8 linearly across the opacity slider, ~0.33 at the default 35%) at 10 dBZ to fully opaque at 60 dBZ along a `pow(t, 2.2)` curve (default: ~0.42 at 30 dBZ, ~0.55 at 40, ~0.64 at 45, ~0.74 at 50). Extinction alone cannot handle widespread stratiform rain — a camera inside 100+ NM of 20–40 dBZ saturates any curve into an opaque wall — so the ceiling guarantees that light and moderate precipitation only tint the scene while heavier cores behind it always have headroom left to read through, and the approach, airport, and terrain stay visible from inside the storm. Front-to-back compositing is otherwise unchanged; heavy cores still saturate and terminate the ray.
- While the camera itself sits in echo (the volume is sampled at the camera position), extinction fades in over the first 5 NM from the camera (`NEAR_FADE_NM`), so a camera flying through rain sees out of it instead of through a veil of the echo around it. A camera in clear air gets no fade, so a storm right beside it keeps its full body. With the ceiling, low in-storm views over live Iowa, Kansas, and New York stratiform payloads keep 70–80% of the background and 12,000 ft views 61–74%, while the same scenes from above read about 1.8–3.3× more opaque than under the previous ramp.
- Color favors the strongest echo along a ray in two ways, while opacity keeps its plain front-to-back accumulation (so a stronger sample never makes a pixel less opaque than the weaker echo in front of it). **MIDA** (after maximum intensity difference accumulation, Bruckner & Gröller 2009): a sample stronger than any the ray has met so far scales the color weight accumulated in front of it by `1 − jump / 10 dBZ` (`MIDA_RANGE_DBZ`, floored at 1/64 so the color stays defined). **Importance-weighted color**: each sample's color weight doubles every 5 dBZ (`COLOR_DOUBLING_DBZ`) while its opacity weight does not. So a pixel's hue follows the strongest echo the ray reached and its opacity follows how much precipitation it crossed. Both exist for the view from above: stratiform columns put 10–20k ft of 15–25 dBZ over the 30–40 dBZ rain beneath, and plain compositing colored those pixels with the light canopy, so top-down views read green over heavy rain. On live New York, Kansas, and Iowa payloads the share of yellow-or-warmer pixels seen from above rose from 2–5% to 17–32%, against 26–41% of echo columns whose column maximum is 25 dBZ or more; opacity coverage in every view is unchanged.
- Sampling is resolution-aware (about one sample per texel crossed, clamped 24–384 steps, now at full source resolution) with a per-pixel jittered start to hide banding. The debug panel's `Volume Bricks` row reports resident bricks, filled logical texels, the cell size, and the coarsening multiple, so the skip rate and any budget-driven coarsening are visible on real weather.
