...
- Per-sample color comes from a nearest-filtered `(band x phase)` LUT texture built from the shared band tables (`buildDbzPhaseLutTexture`), so the discrete legend bands stay crisp inside the volume. Per-sample extinction is cubic in the 5–65 dBZ span (gated to zero below ~5 dBZ so trilinear falloff into empty texels fades cleanly): light precipitation is nearly transparent and heavy cores dominate the integral, so a thick 20 dBZ shell does not bury the 50 dBZ core behind it — at the default 35% opacity a 10 NM deep 20 dBZ shell reads ~10% opaque while a 3 NM 50 dBZ core reads above 50%.
