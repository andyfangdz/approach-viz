...
- Per-sample color comes from a nearest-filtered `(band x phase)` LUT texture built from the shared band tables (`buildDbzPhaseLutTexture`), so the discrete legend bands stay crisp inside the volume. Per-sample extinction follows a 2.75 power of the 5–65 dBZ span (gated to zero below ~5 dBZ so trilinear falloff into empty texels fades cleanly): light precipitation is nearly transparent and heavy cores dominate the integral, so a thick 20 dBZ shell does not bury the 50 dBZ core behind it — at the default 35% opacity a solid 10 NM deep 20 dBZ shell reads ~13% opaque while a 3 NM 50 dBZ core reads ~60%. The ramp was cubic while the volume was rasterized onto 1 NM texels that max-pooled 0.5 NM source cells: that pooling filled the gaps of scattered light echo (a region 50% filled at source resolution read ~94% filled), so light precipitation carried about twice its true optical path and the cubic was tuned against that. At source resolution the same cubic rendered the same scenes 17–28% dimmer, mostly in light and moderate echo; 2.75 restores the previous brightness within 4% on dense (KATL), close-in, and stratiform (KSEA) A/B renders while cores, which saturate, are unchanged.
- Each ray also carries an **opacity ceiling that rises with echo intensity** (`opacityCap` in the shader): the accumulated alpha may not exceed the ceiling for the intensity being sampled, which ramps from a light-echo cap (`uLightOpacityCap`, 0.08–0.8 across the opacity slider, ~0.23 at the default 35%) at 10 dBZ to fully opaque at 60 dBZ along a `pow(t, 2.2)` curve (default: ~0.33 at 30 dBZ, ~0.48 at 40, ~0.58 at 45, ~0.70 at 50). Extinction alone cannot handle widespread stratiform rain — a camera inside 100+ NM of 20–40 dBZ saturates any curve into an opaque wall — so the ceiling guarantees that light and moderate precipitation only tint the scene while heavier cores behind it always have headroom left to read through, and the approach, airport, and terrain stay visible from inside the storm. Front-to-back compositing is otherwise unchanged; heavy cores still saturate and terminate the ray.
