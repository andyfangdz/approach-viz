...
- Extinction also fades in over the first 5 NM from the camera (`NEAR_FADE_NM`), so a camera flying through rain sees out of it instead of through a veil of the echo around it; storms viewed from outside are unaffected. With the ceiling, low in-storm views over live Iowa, Kansas, and New York stratiform payloads keep 70–80% of the background and 12,000 ft views 61–74%, while the same scenes from above read about 1.8–3.3× more opaque than under the previous ramp.
- Compositing favors the strongest echo along a ray in two ways. **MIDA** (maximum intensity difference accumulation, Bruckner & Gröller 2009): a sample stronger than any the ray has met so far scales the color and opacity accumulated in front of it by `1 − jump / 20 dBZ` (`MIDA_RANGE_DBZ`), which reopens opacity headroom for the heavier echo. **Importance-weighted color**: each sample's color weight doubles every 5 dBZ (`COLOR_DOUBLING_DBZ`) while its opacity weight does not, so a pixel's hue follows the strongest echo the ray reached and its opacity still follows how much precipitation it crossed. Both exist for the view from above: stratiform columns put 10–20k ft of 15–25 dBZ over the 30–40 dBZ rain beneath, and plain front-to-back compositing spent the opacity ceiling on that light canopy, so top-down views read green over heavy rain. On live New York, Kansas, and Iowa payloads the share of yellow-or-warmer pixels seen from above rose from 2–5% to 18–35%, against 26–41% of echo columns whose column maximum is 25 dBZ or more; the in-storm opacity figures above are unchanged.
