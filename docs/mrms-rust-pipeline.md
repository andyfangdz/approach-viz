...
4. When R2 publishing is configured, the finished scan is also published as a scan pack for the weather edge Worker (see [weather edge](weather-edge.md)).
5. Next.js route `app/api/weather/nexrad/route.ts` proxies client requests to `v1/weather/volume`, and `app/api/weather/nexrad/echo-tops/route.ts` proxies `v1/weather/echo-tops`, trying the weather edge Worker first when `WEATHER_EDGE_BASE_URL` is set and the runtime otherwise or on failure.
