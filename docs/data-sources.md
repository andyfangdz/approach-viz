...
- Startup drops objects earlier releases created that nothing reads: trigger-maintained `R*Tree` mirrors of tracks and points, point time/hex indexes, the extra track indexes, and pre-ring `traffic_points_p*` partition tables. Expired ring slots are recycled by dropping and recreating their table, which also replaces the older `AUTOINCREMENT` table definition. Freed pages stay in the file's freelist and are reused.
