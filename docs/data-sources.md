...
- One writer thread owns the SQLite connection. It keeps a mirror of the persisted track rows, merges each snapshot against that mirror instead of reading tracks back, skips rows whose merged state is unchanged, and advances the mirror only after the transaction commits. Transient lock errors retry the SQLite transaction only; the memory merge runs once per snapshot.
- Startup drops the maintenance earlier releases attached: the triggers that kept `R*Tree` mirrors of tracks and points current, the track `R*Tree`, and the extra track indexes. These are schema-only or track-sized changes, so startup stays fast. Freeing a large table reads every page of it, which on the production volume took longer than the 60-second deploy health window. Each ring slot's legacy `R*Tree` and time/hex indexes are dropped when the slot is next recycled. Expired slots are recycled by dropping and recreating their table, which also replaces the older `AUTOINCREMENT` definition, so all 12 slots are clean within an hour. Pre-ring `traffic_points`/`traffic_points_p*` tables and `traffic_partitions` are neither written nor read and are left for an offline cleanup (drop them with the service stopped, then `VACUUM`). Freed pages stay in the file's freelist and are reused.
...
- The service fetches/decode-checks all reflectivity levels (`00.50..19.00 km`) plus level-matched dual-pol products (`MergedZdr_<level>`, `MergedRhoHV_<level>`), decodes GRIB2 (structure through the Rust `grib` crate, PNG-packed samples through an exact lookup-table/arithmetic path), computes phase-coded voxels, ingests direct echo-top products (`EchoTop_18_00.50`, `EchoTop_30_00.50`, `EchoTop_50_00.50`, `EchoTop_60_00.50`), and stores compact zstd-compressed snapshots.
...
- NOAA publishes a scan in waves (base level, then most levels ~30 s later, the highest ~95 s after the key timestamp), so the ingest waits per object for up to a 180 s publication window and pending timestamps are ingested newest-first. On ~8% of scans some reflectivity levels are stamped 1-6 s off the base level's, so a level's file is found by exact key or, failing that, the neighbor stamped within 10 s. Dual-pol products publish every 5 minutes with one stamp for all levels.
...
- Echo-top responses are always AVET binary (`application/vnd.approach-viz.echo-tops.v3`); there is no JSON variant.
