...
- Dual-pol fields are fetched for the same timestamp and altitude slice as reflectivity when available; legacy `PrecipFlag_00.00` + `Model_0degC_Height_00.50` is also sampled for correction when dual-pol returns `mixed`, and `PrecipFlag_00.00` snow can override dual-pol rain/mixed in snow-biased conditions. When dual-pol is sparse/lagging beyond 5 minutes ingest switches to latest available aux timestamps with explicit legacy fallback phase resolution.
