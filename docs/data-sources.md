...
- Dual-pol fields are fetched for the same timestamp and altitude slice as reflectivity when available; when dual-pol is sparse/lagging beyond 5 minutes, ingest switches to the latest available aux timestamps and enables legacy `PrecipFlag_00.00` + `Model_0degC_Height_00.50` fallback phase resolution.
