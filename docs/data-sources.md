...
- Proxy converts decoded MRMS cells to request-origin local NM 3D voxels with per-level altitude bounds, dataset-derived X/Y footprint, and per-voxel phase code (rain/mixed/snow), then applies dBZ threshold, AOI range culling, and voxel-count decimation.
