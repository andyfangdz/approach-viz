...
- Weather edge changes (scan pack format, `mrms_query`, the Worker) are checked with the `scan_pack` example, which proves 2,040 pack-served queries byte-identical to the snapshot path on a real scan and reports the peak heap of one pack query, and with the Worker tests in `services/weather-edge` (`npm run build:wasm && npm test`). See [weather edge](weather-edge.md#validation).
