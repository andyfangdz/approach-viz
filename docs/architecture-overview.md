...
flowchart LR
  ...
  subgraph edge["Cloudflare"]
    WE["weather-edge Worker<br/>/v1/weather/*"] --> R2[("R2 scan packs")]
  end
  RS -- "publish scans" --> R2
  PROXY -- "weather, first" --> WE
  PROXY --> RS
  APP --> RS
...
- [Weather edge](weather-edge.md): R2 scan packs, the Cloudflare Worker, and the proxy's edge-first fallback.
