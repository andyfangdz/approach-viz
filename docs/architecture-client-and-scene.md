...
- `app/scene/NexradVolumeOverlay.tsx` polls MRMS weather (proxy by default, optional direct upstream URL), requests wire version 2 by default, decodes v1/v2 compact binary payloads (including merged-brick span records), applies optional curvature compensation, and renders instanced volumetric weather voxels as an optional overlay group without client-side voxel decimation.
