...
- Approach-path altitude resolution, path geometry assembly, and hold geometry generation now live in one shared Rust implementation (`crates/approach-viz-core/src/approach_path.rs`), exercised in Rust tests and in web WASM tests (`app/scene/approach-path/geometry-rust.test.ts`).
