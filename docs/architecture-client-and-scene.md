...
- `app/scene/NexradVolumeOverlay.tsx` polls MRMS reflectivity + echo-top products (proxy by default, optional direct upstream URL), decodes compact MRMS binary payloads through dedicated worker pipelines (decode + prepare, with synchronous fallback only if worker paths fail), records worker-failure diagnostics (stage/message/timestamp) for debug-panel visibility, routes heavier post-decode preprocessing (`prepare-volume`/`prepare-echo-top`) through an isolated dedicated worker channel to avoid decode-queue starvation, applies optional curvature compensation, and renders instanced volumetric weather voxels plus thresholded echo-top caps/altitude guides/cross-section aids as optional overlay groups without client-side voxel decimation.
