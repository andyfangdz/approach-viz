...
- `app/scene/approach-path/PathTube.tsx` receives the swept tube (split at the minimums in the worker, `app/scene/approach-path/path-tube.ts`) and the dashed below-minimums polyline as transferable buffers; it only wraps them in geometry.
- `app/scene/LiveTrafficOverlay.tsx` owns poll/backfill scheduling; its traffic worker performs fetch, binary decode, merge/prune, projection, and draw-buffer assembly. Trail segments, marker matrices, and heading ticks return upload-ready through Comlink transferables, and render hashes avoid redundant uploads. Worker failures surface in UI/debug state without synchronous fallback.
...
- `app/scene/labels/SceneLabels.tsx` draws all in-scene text (waypoints, airports/runways, holds, turn constraints, callsigns, obstacles, altitude guides, storm tops) as instanced, shader-billboarded quads over a worker-rasterized atlas. Do not add drei `<Html>` labels to the scene; they update every frame.
- `app/scene/geometry/scene-geometry.worker.ts` builds the terrain wireframe mesh, holds Terrarium elevation rasters for the weather layers (`useElevationRaster` returns a handle, not a sampler), samples the volume ground heightfield and mosaic drape, and extrudes airspace.
- `app/app-client/SceneCanvas.tsx` renders on demand (`frameloop="demand"`); imperative scene mutations call `invalidate()`. `AsyncShaderCompiler` links new materials off the main thread before their first draw, and `SceneEnvironment` decodes the HDR environment in a worker.
