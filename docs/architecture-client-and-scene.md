...
- Optional live traffic state (enable flag + hide-ground toggle + callsign-label toggle + hide-ground-callsign-label toggle + history retention minutes) is managed in `app/AppClient.tsx` and fed into `SceneCanvas`/`OptionsPanel`.
...
- Options-panel state (camera control mode, vertical scale, terrain radius, bathymetry, traffic toggles/history window, MRMS weather toggles/threshold/opacity/declutter/slice controls) is persisted in browser `localStorage` and restored on client startup.
...
- `app/scene/LiveTrafficOverlay.tsx` handles ADS-B polling and offloads track merge/prune/projection to a dedicated traffic worker (`app/scene/traffic/traffic.worker.ts`) with synchronous in-thread fallback; trail/heading visuals are batched into shared `lineSegments` geometries, and worker render-hash diffing avoids redundant main-thread track updates.
