...
- A teardrop/course-reversal (course-from-fix outbound + a no-fix `CI`/`VI` intercept, for example the `KDDC I14` `FLACK` transition at `OWENJ`) is rendered as a **single smooth circular arc** through the outbound fix, the outbound apex, and the roll-out fix — one continuous curve with no straight outbound leg, terminating on the roll-out fix where it meets the final approach course. The roll-out fix is supplied by the scene composition (web `app/scene/ApproachPath.tsx` and iOS `ApproachPathGeometry.swift`), which appends the final approach's first fix (the IF where the final segment begins, carrying the published inbound course) to a transition ending in `CI`/`VI` before geometry assembly; the shared engine (`build_arc_through_three_points`) then draws the arc and the transition terminates on that fix.
