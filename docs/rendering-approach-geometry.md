...
- Course-from-fix legs (`FA`/`FC`/`FD`/`FM`) originate at a named fix and proceed outbound along the published course; they are drawn as an outbound segment from the fix (using the published leg distance for `FC`/`FD`, falling back to a default outbound length for `FA`/`FM`) rather than collapsing back onto the fix waypoint. Paired with a downstream `CI` course-to-intercept turn, this renders the outbound leg of a teardrop/course-reversal (for example the `KDDC I14` `FLACK` transition teardrop at `OWENJ`, previously drawn as a stub at the fix).
- The inbound side of such a teardrop/course-reversal rejoins the final approach course: when a transition ends in a no-fix intercept leg (`CI`/`VI`) it has nothing downstream to intercept, so the scene composition (web `app/scene/ApproachPath.tsx` and iOS `ApproachPathGeometry.swift`) appends the final approach's first course-carrying fix leg (the FAF/localizer leg) to that transition before geometry assembly. The shared engine's heading-to-course intercept then turns the reversal back onto the published final approach course and merges with the final segment in one continuous turn (rather than flying the literal `CI` heading, which diverges from the localizer). Appending the localizer's downstream fix (the FAF) — not the IF, which sits at the reversal apex and leaves no room to intercept — keeps the inbound leg on the course.
- As an engine-level fallback (when no final approach course is available to append), a terminal `CI`/`VI` intercept immediately after a course-from-fix outbound leg is drawn as a single broad, continuous turn (its own wider radius band scaled to the outbound leg length, not the tight `VI` heading-stub radius that renders a sharp spike) plus an inbound leg along the published intercept course mirroring the outbound distance (capped), so it still completes the reversal instead of dead-ending at a stub.
