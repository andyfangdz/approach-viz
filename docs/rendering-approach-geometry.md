...
- The inbound side of such a teardrop/course-reversal is completed in geometry: when a no-fix intercept leg (`CI`/`VI`) terminates a segment immediately after a course-from-fix outbound leg, the reversal turn arc is followed by an inbound leg drawn along the published intercept course, mirroring the outbound distance (capped) so the path turns back toward the inbound approach course instead of dead-ending at a short turn stub. The inbound leg parallels/rejoins the final approach course depicted in the adjacent final segment.
