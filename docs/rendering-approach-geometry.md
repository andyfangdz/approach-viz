...
- The inbound side of such a teardrop/course-reversal is completed in geometry: when a no-fix intercept leg (`CI`/`VI`) terminates a segment immediately after a course-from-fix outbound leg, the reversal is drawn as a single broad, continuous turn (its own wider radius band scaled to the outbound leg length, not the tight `VI` heading-stub radius that renders a sharp spike) followed by an inbound leg along the published intercept course, mirroring the outbound distance (capped). The outbound leg curves continuously into the inbound leg, which parallels/rejoins the final approach course depicted in the adjacent final segment instead of dead-ending at a short turn stub.
