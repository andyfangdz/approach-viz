...
- MRMS voxel coloring is phase-aware (rain/mixed/snow): server-side phase codes use a thermodynamic-first resolver (precip flag, freezing level, wet-bulb/surface temperature, bright-band heights, optional RQI) and then apply level-matched MRMS dual-pol (`MergedZdr`, `MergedRhoHV`) as weighted correction; mixed output is retained when transition or competing rain/snow cues are present and only suppressed when mixed separation is weak, while stale/sparse dual-pol (>5 minutes) is down-weighted with explicit fallback telemetry.
