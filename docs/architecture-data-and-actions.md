...
- Current FAA CIFP is the normal build/runtime geometry source of truth. `build-db` conditionally adds the versioned FAA-cycle-`260806` `KSBS / R32-Z` historical fixture only when current CIFP no longer contains that exact procedure ID. A live/current row therefore always takes precedence.
...
- Selector data merges CIFP procedures, the conditional historical KSBS fallback, and minima/plate-only procedures missing CIFP geometry. Historical fallback and external-only provenance remain distinct in the scene-action API.
...
- Query endpoint (`/v1/weather/volume`, with legacy `/v1/volume` alias) loads latest snapshot in memory and performs fast request-origin filtering (`lat/lon/minDbz/maxRangeNm`) with tile-indexed voxel subsets before serializing AVMR v5 FlatBuffers responses (`application/vnd.approach-viz.mrms.v5`).
