...
- Both weather proxies delegate to `runtime-proxy.ts`: one 8-second deadline spans the upstream requests plus response-body consumption. When `WEATHER_EDGE_BASE_URL` is set, the weather edge Worker is tried first (4-second share) and any failure falls back to the runtime with the same query; `X-AV-WEATHER-UPSTREAM` names the upstream that answered (see [weather edge](weather-edge.md)). Invalid numeric parameters return 400; failure of every upstream returns 502; deadline expiry returns 504. A canonical 404 is final for that upstream; there is no legacy-path retry.
