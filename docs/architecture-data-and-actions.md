...
- Live ADS-B traffic decode/query runs in the Rust runtime service endpoint `/v1/traffic/adsbx`; Next.js route `app/api/traffic/adsbx/route.ts` is a same-origin proxy that preserves binary payloads. When the runtime fails or returns a 5xx for a binary request, the route fetches the ADS-B Exchange binCraft snapshot for the request's box itself and answers with current aircraft only (`X-AV-TRAFFIC-UPSTREAM: direct`), using the runtime's decode/selection/encoding code through the server WASM (see [runtime fallbacks](runtime-fallbacks.md#traffic)).
...
- The store keeps per-aircraft track state plus append-only point history; `/v1/traffic/adsbx` serves current tracks and history from memory, with SQLite for persistence/restart recovery. Both storage paths share `merge_track` and its timestamp/field/history-sampling rules. Memory publishes before the SQLite transaction; a failed transaction leaves memory live, and the next ingest merges against each store's own state.
...
- Ingestion fetches/decode-checks all configured reflectivity levels (`00.50..19.00 km`) plus level-matched dual-pol auxiliaries (`MergedZdr_<level>`, `MergedRhoHV_<level>`), decodes GRIB2 through the Rust `grib` crate for structure and an exact lookup-table/arithmetic path for PNG-packed fields (the crate is the fallback for other layouts), ingests direct echo-top products (`EchoTop_18/30/50/60`), resolves per-voxel phase server-side, and persists compact zstd snapshot files.
...
- Pending timestamps are ingested newest-first: a completed ingest drops every older pending timestamp, so a backlog (cold start, outage) costs one ingest. Each level waits for NOAA to publish its objects inside a 180 s publication window (and accepts a neighbor stamp within 10 s, since some levels of ~8% of scans are stamped off the base level's) rather than failing and restarting the scan, so delayed levels never repeat finished work; startup bootstrap enqueues a deeper recent-key window and the newest complete cycle wins.
- Query endpoint (`/v1/weather/volume`) loads latest snapshot in memory and performs fast request-origin filtering (`lat/lon/minDbz/maxRangeNm`) with tile-indexed voxel subsets before serializing AVMR v5 FlatBuffers responses (`application/vnd.approach-viz.mrms.v5`).
...
- Both weather proxies delegate to `runtime-proxy.ts`: one 8-second deadline spans every attempt plus response-body consumption. With `WEATHER_R2_*` set, the route answers from the runtime's R2 scan packs (`scan-packs.ts`, through the server WASM) while the published scan is under 15 minutes old, with the runtime as fallback; otherwise the runtime goes first. The first source gets a 4-second share. `X-AV-WEATHER-UPSTREAM` names the source that answered, and successful responses are CDN-cached for 30 s (see [runtime fallbacks](runtime-fallbacks.md)). Invalid numeric parameters return 400; failure of every source returns 502; deadline expiry returns 504. A canonical 404 is final for that source; there is no legacy-path retry.
