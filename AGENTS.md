...
- Stack: Next.js 16 (App Router, React Compiler), React, TypeScript, react-three-fiber, SwiftUI, MetalKit, GRDB.swift, Nuke, FloatingPanel, SQLite, Rust (Cargo workspace with `approach-viz-core` + `approach-viz-runtime` + `uniffi-bindgen-swift`), AWS SNS/SQS, Datadog.
...
- `npm run test:ios` now invokes `xcodebuild -skipMacroValidation` because the current `Swift Navigation` package release pulls in Swift package macros (`CasePathsMacros` / `PerceptionMacros`) that otherwise require per-machine trust before scripted builds can proceed.
