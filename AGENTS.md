...
- The native detail view now exposes floating SwiftUI Layers, Options, and Debug panels controlled by FAB buttons instead of a fixed bottom inset. The Layers panel toggles `approach`, `airspace`, and `ADS-B Traffic`, the Options panel includes the same ADS-B controls currently exposed on the web client (`Hide Ground Traffic`, `Show Traffic Callsigns`, `Hide Ground Callsign Labels`, `Traffic History`, `Show Departed Traffic Trails`) plus vertical scale, and the Debug FAB opens a Metal render-stats panel instead of keeping the stats overlay permanently visible.
