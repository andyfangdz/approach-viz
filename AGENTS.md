...
- Runtime weather note: MRMS ingest/query moved to `services/mrms-rs/` (Rust Axum service). It consumes SNS->SQS new-object events, ingests/decodes complete scans once, uses the Rust `grib` crate for GRIB2 template decoding (including PNG-packed fields), stores zstd snapshots with 5 GB retention, and serves query-filtered binary voxel payloads (`application/vnd.approach-viz.mrms.v1`) through the OCI host. `app/api/weather/nexrad/route.ts` is now a thin proxy to that upstream service. Phase inputs prefer level-matched dual-pol fields (`MergedZdr_<level>`, `MergedRhoHV_<level>`) at the reflectivity timestamp; when aux lags/sparsity exceeds a 5-minute threshold, ingest uses the latest available aux cycle plus legacy `PrecipFlag_00.00` / `Model_0degC_Height_00.50` fallback classification. Legacy phase is also used as a correction path when dual-pol resolves `mixed` and when `PrecipFlag_00.00` indicates snow (snow-bias override). Scheduler retries pick the earliest due pending timestamp (not newest-first) to avoid starvation when aux products lag, and bootstrap enqueue scans the most recent 120 base keys so restart catches delayed-complete cycles.
...
- [`docs/mrms-phase-methodology.md`](docs/mrms-phase-methodology.md) — dual-pol phase-classification thresholds, stale-aux fallback policy, and debug telemetry semantics
...
MRMS volume intensity uses phase-aware reflectivity coloring (rain/mixed/snow): server-side phase resolution prefers level-matched `MergedZdr` + `MergedRhoHV` thresholds (low RhoHV -> mixed, otherwise ZDR separates rain vs snow), then applies legacy phase correction for mixed outputs and a precip-flag snow bias (`PrecipFlag_00.00` snow can override dual-pol rain/mixed) before rendering phase-specific aviation palettes; when dual-pol is stale/sparse it falls back to latest available aux plus legacy classification. Rendering still uses clip-safe color gain (preserves hue, avoids distant whitening) in a dual-pass volume (`NormalBlending` front-face base + additive glow, both `depthWrite=false`) with per-instance dBZ-driven alpha (power-curve mapping so low-intensity echoes are nearly transparent while high-intensity cores remain prominent) plus a configurable master-opacity slider (opacity updates apply in place).
