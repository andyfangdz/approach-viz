...
- Deploy Rust runtime service to OCI host: `RUNTIME_MRMS_SQS_QUEUE_URL=... scripts/runtime/deploy_oci.sh ubuntu@100.86.128.122` (script stages and replaces remote source to avoid stale files, excludes local `target/` artifacts from upload, then waits for local `/healthz` readiness after restart before final `/v1/meta` smoke check)
