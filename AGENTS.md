...
- Compute: Cargo workspace containing `approach-viz-core`, `approach-viz-runtime`, `approach-viz-server-wasm`, and `uniffi-bindgen-swift`. Core is shared through WASM and UniFFI.
- Data/operations: SQLite, FAA CIFP/D-TPP/DOF, MRMS SNS/SQS, ADSB Exchange, OCI runtime, Cloudflare R2 (weather scan packs), Datadog.
...
- Keep expensive web computation in workers. There is no synchronous fallback for approach geometry (including path tubes), MRMS prepare/decode, traffic processing and draw buffers, selector filtering, chart tile decode/compositing, terrain meshes, elevation sampling, airspace extrusion, label rasterization, or the HDR environment decode.
- The web canvas renders on demand. Code that changes the scene outside React props (instance uploads, uniforms, texture copies, material patches) calls `invalidate()`. In-scene text goes through `SceneLabels`, never drei `<Html>`, which re-projects every frame.
...
| Server WASM build (committed)    | `npm run build:wasm:server`                                        |
| Scan pack equivalence            | `cargo run --release -p approach-viz-runtime --example scan_pack`  |
...
The server WASM (`packages/approach-viz-server-wasm/`, from `crates/approach-viz-server-wasm`) is committed like the browser WASM because the Vercel build has no Rust toolchain; rebuild it after changing `mrms_query`, `mrms_pack`, `traffic_query`, or that crate. The `scan_pack` example takes a stored `*.avsn.zst` snapshot and proves pack-served queries byte-identical to the snapshot path. Route tests use `fixtures/server-wasm/`, kept current by core tests (`UPDATE_SERVER_FIXTURES=1`). See [runtime fallbacks](docs/runtime-fallbacks.md).
...
- `app/scene/geometry/`, `app/scene/labels/`, `app/scene/shared/async-shader-compiler.tsx`: the scene-geometry worker (terrain mesh, Terrarium rasters and all elevation sampling, airspace), the worker-rasterized label system, and async shader linking before first draw. See [performance](docs/rendering-performance.md) and [worker protocols](docs/worker-transport-protocols.md).
...
- `services/runtime-rs/`: MRMS ingest/query and ADS-B cache/query. The binary calls the library’s `run()` entry point; service modules and their tests have one compilation owner. MRMS ingest runs one task per level that waits for NOAA publication and reduces decoded grids to sparse voxel inputs; PNG-packed GRIB2 decodes through an exact lookup path that must stay bit-identical to the `grib` crate, and a scan is published before it is persisted. Prove ingest optimizations with the one-shot profile's `Scan fingerprint` (unchanged before/after). Responses are gzip level 3 on the zlib-rs backend because compression, not payload construction, dominated query CPU; profile with `perf` before optimizing (`services/runtime-rs` release builds keep debug info). Traffic memory and SQLite advance independently but share merge/history-sampling policy; the writer merges against its committed mirror of SQLite rather than reading rows back. Memory remains live after a failed persistence transaction; later ingests retry persistence. Prove traffic store changes with `cargo bench --bench traffic_store` (fingerprints unchanged old vs new; `--legacy-schema` for the production schema). Startup must not free large tables: on the production volume that reads every page and exceeds the 60-second deploy health window.
- `crates/approach-viz-core/src/mrms_query/` and `mrms_pack.rs`: MRMS query validation, windowing, brick merging, AVMR/AVET encoding, and the R2 scan pack format. The runtime (in-memory snapshot) and the web weather route (packs in R2, through `crates/approach-viz-server-wasm`) both answer weather queries through this code and must stay byte-identical; tile and echo-top order are part of that contract.
- `crates/approach-viz-core/src/traffic_query.rs`: traffic query parsing, binCraft decoding, current-aircraft ranking, and AVTR encoding, shared by the runtime and the web traffic route's direct fallback. The fallback's selection must equal what the runtime store reports for a single poll (`direct_fallback_matches_a_store_holding_one_poll`); prove traffic refactors with the `traffic_store` bench fingerprints.
...
- Runtime wire contracts: `/v1/weather/volume` uses AVMR v5; `/v1/weather/echo-tops` always returns AVET v3 (no JSON variant; `Accept` is ignored); `/v1/traffic/adsbx` uses JSON or AVTR v4 via `format=binary`. The legacy weather aliases `/v1/volume` and `/v1/echo-tops` were removed. With `RUNTIME_R2_*` configured (host env file `/etc/approach-viz-runtime/r2-publish.env`), each scan is published as an `AVSP` v1 scan pack plus `<prefix>/latest.json`; all four variables or none, and the manifest never moves backwards.
- Web weather proxies share an 8-second deadline covering every attempt and the body read. With `WEATHER_R2_*` set (all or none) they answer from R2 scan packs while the published scan is under 15 minutes old and fall back to the runtime, or the reverse when stale; malformed queries return 400, failure of every source 502, and deadline expiry 504. The web traffic proxy answers binary requests directly from ADS-B Exchange (current aircraft, no history) when the runtime fails or returns a 5xx. The native app calls the runtime directly and has neither fallback. FAA PDF proxy limits and ETag semantics are documented separately.
...
- Weather: [Rust pipeline](docs/mrms-rust-pipeline.md), [runtime fallbacks (R2 packs, direct traffic)](docs/runtime-fallbacks.md), [phase methodology](docs/mrms-phase-methodology.md), [volume rendering](docs/rendering-weather-volume.md), [storm cells](docs/rendering-storm-cells.md).
