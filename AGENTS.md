...
- Runtime endpoints:
  - `GET /v1/weather/volume` -> `application/vnd.approach-viz.mrms.v3` (legacy alias `/v1/volume`)
  - `GET /v1/weather/echo-tops` -> JSON by default; AVET binary with `Accept: application/vnd.approach-viz.echo-tops.v1` (legacy alias `/v1/echo-tops`)
  - `GET /v1/traffic/adsbx` -> JSON or binary (`format=binary`, `application/vnd.approach-viz.traffic.v1`)
