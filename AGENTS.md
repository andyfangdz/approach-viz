...
- Runtime wire contracts: `/v1/weather/volume` uses AVMR v5; `/v1/weather/echo-tops` always returns AVET v3 (no JSON variant; `Accept` is ignored); `/v1/traffic/adsbx` uses JSON or AVTR v4 via `format=binary`. The legacy weather aliases `/v1/volume` and `/v1/echo-tops` were removed.
- Web weather proxies share an 8-second deadline covering the upstream fetch and body read; malformed queries return 400, upstream/read failures 502, and deadline expiry 504. FAA PDF proxy limits and ETag semantics are documented separately.
