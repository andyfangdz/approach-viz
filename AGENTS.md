...
- Deploy Rust runtime service to OCI host: `RUNTIME_MRMS_SQS_QUEUE_URL=... scripts/runtime/deploy_oci.sh ubuntu@100.86.128.122` (default remote build mode stages and replaces remote source to avoid stale files, excludes local `target/` artifacts from upload, then builds on host; optional local cross-compile mode uses `RUNTIME_DEPLOY_BUILD_MODE=local-cross` with `cargo zigbuild` or `cross` installed locally to build `aarch64-unknown-linux-gnu` and upload only the binary before restart; both modes wait for local `/healthz` readiness after restart before final `/v1/meta` smoke check)
