...
- MRMS base/glow volume rendering uses flat TypedArrays and direct InstancedMesh matrix assignments to bypass object allocation and full rotation overhead, shares populated instanced transforms/colors between passes, uses grow-only instanced capacity buckets, avoids per-refresh index remapping allocations by reusing scratch index buffers, computes threshold/declutter/cross-section/echo-top prep in worker tasks before main-thread mesh writes, and uses SharedArrayBuffer + Atomics transport for prepared-volume worker responses with overflow-driven shared-capacity growth/retry.
- MRMS decode worker requests transfer fetched binary `ArrayBuffer` payloads from main thread and decode responses transfer typed-array buffers back (including flattened echo-top typed arrays) to reduce cross-thread clone overhead.
