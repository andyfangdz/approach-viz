...
- Build WASM core module (wasm-pack + copy to `public/`): `npm run build:wasm` (requires `wasm-pack`; outputs to `packages/approach-viz-core-wasm/` and copies `.wasm` binary + JS glue to `public/` for worker `fetch()` loading and Playwright smoke tests; all wasm-pack artifacts are checked in so CI/Vercel don't require Rust toolchain)
...
- `crates/approach-viz-core/` — shared Rust core crate (compiles to native `rlib` + `cdylib` for wasm32); modules: `coords`, `types`, `echo_top_wire_codec`, `mrms_wire_codec`, `traffic_codec`, `mrms_preprocess`, `wire_helpers`; optional `wasm` feature gates `wasm-bindgen`
...
- `packages/approach-viz-core-wasm/` — wasm-pack build output (checked in for CI/Vercel); regenerated by `npm run build:wasm`, consumed by worker modules via `app/scene/shared/wasm-loader.ts`
...
- MRMS worker poll responses include transferable decoded volume typed arrays for mesh uploads and prepared echo-top surfaces + summary metadata (all decoded and prepared in single WASM calls), so fetch/decode/prepare all stay off the main thread.
