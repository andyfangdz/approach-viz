...
| WASM browser smoke (localhost:3000) | `npm run test:smoke`                                               |
...
- `services/runtime-rs/`: MRMS ingest/query and ADS-B cache/query. Traffic memory and SQLite advance independently but share merge/history-sampling policy. Memory remains live after a failed persistence transaction; later ingests retry persistence.
