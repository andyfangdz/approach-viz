...
- Runtime weather note: MRMS ingest/query moved to `services/mrms-rs/` (Rust Axum service). It consumes SNS->SQS new-object events, ingests/decodes complete scans once, uses the Rust `grib` crate for GRIB2 template decoding (including PNG-packed fields), stores zstd snapshots with 5 GB retention, and serves query-filtered binary voxel payloads (`application/vnd.approach-viz.mrms.v1`) through the OCI host. `app/api/weather/nexrad/route.ts` is now a thin proxy to that upstream service. Phase inputs prefer level-matched dual-pol fields (`MergedZdr_<level>`, `MergedRhoHV_<level>`) at the reflectivity timestamp; when aux lags/sparsity exceeds a 5-minute threshold, ingest uses the latest available aux cycle plus legacy `PrecipFlag_00.00` / `Model_0degC_Height_00.50` fallback classification. Scheduler retries pick the earliest due pending timestamp (not newest-first) to avoid starvation when aux products lag, and bootstrap enqueue scans the most recent 120 base keys so restart catches delayed-complete cycles.
