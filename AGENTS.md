...
- Compute: Cargo workspace containing `approach-viz-core`, `approach-viz-runtime`, `approach-viz-weather-edge`, and `uniffi-bindgen-swift`. Core is shared through WASM and UniFFI.
- Data/operations: SQLite, FAA CIFP/D-TPP/DOF, MRMS SNS/SQS, ADSB Exchange, OCI runtime, Cloudflare R2 + Workers (weather edge), Datadog.
...
| Weather edge Worker              | `cd services/weather-edge && npm run build:wasm && npm test`       |
| Weather edge deploy              | `cd services/weather-edge && npm run deploy`                       |
...
The weather edge Worker (`services/weather-edge`) has its own `package.json` and lockfile, excluded from the root typecheck; its tests need the WASM built first and run in CI's `weather-edge` job. The `scan_pack` example takes a stored `*.avsn.zst` snapshot and proves pack-served queries byte-identical to the snapshot path; run it after changing `mrms_query`, `mrms_pack`, or the Worker's WASM crate. See [weather edge](docs/weather-edge.md).
...
- `crates/approach-viz-core/src/mrms_query/` and `mrms_pack.rs`: MRMS query validation, windowing, brick merging, AVMR/AVET encoding, and the R2 scan pack format. The runtime (in-memory snapshot) and `services/weather-edge` (packs in R2, through `crates/approach-viz-weather-edge`) both answer weather queries through this code and must stay byte-identical; tile and echo-top order are part of that contract. The Worker refuses windows over its WASM memory budget with a 503 so the proxy falls back to the runtime.
...
- Runtime wire contracts: `/v1/weather/volume` uses AVMR v5; `/v1/weather/echo-tops` always returns AVET v3 (no JSON variant; `Accept` is ignored); `/v1/traffic/adsbx` uses JSON or AVTR v4 via `format=binary`. The legacy weather aliases `/v1/volume` and `/v1/echo-tops` were removed. With `RUNTIME_R2_*` configured (host env file `/etc/approach-viz-runtime/edge-publish.env`), each scan is published as an `AVSP` v1 scan pack plus `<prefix>/latest.json`; all four variables or none, and the manifest never moves backwards.
- Web weather proxies share an 8-second deadline covering the upstream fetches and body read. With `WEATHER_EDGE_BASE_URL` set they try the weather edge Worker first, then the runtime; malformed queries return 400, failure of every upstream 502, and deadline expiry 504. FAA PDF proxy limits and ETag semantics are documented separately.
...
- Weather: [Rust pipeline](docs/mrms-rust-pipeline.md), [edge (R2 + Worker)](docs/weather-edge.md), [phase methodology](docs/mrms-phase-methodology.md), [volume rendering](docs/rendering-weather-volume.md), [storm cells](docs/rendering-storm-cells.md).
